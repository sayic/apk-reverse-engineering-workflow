---
name: art
description: "The user requests organizing extracted art resources and importing them into Unity."
---
...
- Steps:
  ...
  - Update context at task start, stage completion/failure, and finish, recording artifact/index locations. Reuse the main task record when called by another skill.
...
- Registered tools and special texture experience:
  - Call a compatible local AssetRipper through [run_assetripper_export](../../apkbreak_tools/common/run_assetripper_export.md), explicitly providing the service address, settings, and tool version. generated_unverified indicates only candidate results.
  - Use [inspect_unity_assets](../../apkbreak_tools/common/inspect_unity_assets.md) for supporting static checks and retain its unchecked items. It does not validate Mesh buffers, audio/video, or Unity runtime behavior.
