---
name: import
description: "The user requests copying specified art outputs and their required dependencies into another Unity project."
---
...
- Steps:
  ...
  7. Record source/target paths, copy lists, adaptations, conflicts, checks, and rollback locations. Reuse the main task record when called by another skill.
...
- Registered tools:
  - Use [verify_meta_guids](../../apkbreak_tools/common/verify_meta_guids.md) when source/target relative layouts match. For intentional path/GUID changes, check actual mappings rather than assuming one-to-one paths.
  - Use [inspect_unity_assets](../../apkbreak_tools/common/inspect_unity_assets.md) to support target static-reference checks. Unknown references remain unresolved, not intentionally excluded just because source directories were not copied. Add only confirmed external dependencies to known GUID lists.
