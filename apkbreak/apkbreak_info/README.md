...
Copy [apk.csv](templates/apk.csv) into this directory, name it after the package's main filename, and fill in one data row. The template contains only headers. For inputs with the same name but different hashes, append a hash suffix according to the naming rules.
